...
func (f *Future) WaitGetResp() *Resp {
	return <-f.resp
}
...
func (f *Future) GetResp() *Resp {
	...
	case <-time.After(1000 * time.Millisecond):
		return newRespIOErr(RedisTimeOutErr)
	}
}

var RedisRespTimeoutError = errors.New("redis get resp time out with")

func (f *Future) TryGet(t time.Duration) (resp *Resp, err error) {
	sig := make(chan bool, 1)
	time.AfterFunc(t, func() { sig <- false })
	select {
	case resp = <-f.resp:
		return
	case <-sig:
		err = RedisRespTimeoutError
		return
	}
}
...
func (f *Future) SendResp(resp *Resp) {
	select {
	case f.resp <- resp:
		return
	default:
		fmt.Println("cant not send resp", resp.String())
	}
}
...
func (rr *RespReader) beginTask() {
	...
	defer rr.client.closeWithError(errors.New("resp reader error"))
	...
	for rr.client.isWorking {
		...
		default:
			...
			if err != nil {
				rr.client.LastCritical = err
				go rr.client.closeWithError(err)
				return
			}
